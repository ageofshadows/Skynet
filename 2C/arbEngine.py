...
from vnpy.trader.app.ctaSpreadStrategy.strategy import STRATEGY_CLASS
...
########################################################################
class ArbEngine(object):
    """ARB策略引擎"""

    #----------------------------------------------------------------------
    def __init__(self, mainEngine, eventEngine, settingFileName = 'ARB_setting.json', onLine=False):
        """Constructor"""
    ...
    #----------------------------------------------------------------------
    def sendIBOrder(self, vtSymbol, orderType, price, volume, strategy, isMktOrder=False):
	"""IB 发单"""
	...
	if orderType == CTAORDER_BUY or orderType == CTAORDER_COVER :
        ...
        self.writeArbLog(u'策略%s发送委托，%s，%s，%s@%s' 
	...
	if contract.gatewayName == "IB":
	    return self.sendIBOrder(vtSymbol, orderType, price, volume, strategy, False)	
    ...
    def confSettle(self, vtSymbol):
        """确认结算单"""
        #contract = self.mainEngine.getContract(vtSymbol)
        #self.mainEngine.confSettle(contract.gatewayName)    # 确认结算单
	pass
	...
	# 推送tick到对应的策略实例进行处理
	if tick.symbol in self.tickStrategyDict:
	    # 添加datetime字段
    ...
    def processOrderEvent(self, event):
        ...
        if not self.onLineCompare and order.vtOrderID in self.orderStrategyDict:
            strategy = self.orderStrategyDict[order.vtOrderID]            
	    self.callStrategyFunc(strategy, strategy.onOrder, order)
	if self.onLineCompare:
	    if order.symbol in self.tickStrategyDict:
		l = self.tickStrategyDict[order.symbol]
		for strategy in l:
			self.callStrategyFunc(strategy, strategy.onOrder, order)
	...
	# 将成交推送到策略对象中
	if not self.onLineCompare and trade.vtOrderID in self.orderStrategyDict:
    ...
    def loadStrategy(self, setting):
        ...
        try:
            ...
            self.writeArbLog(u'载入策略出错：%s' %e)
            return
        
        # 获取策略类
        strategyClass = STRATEGY_CLASS.get(className, None)
        if not strategyClass:
            self.writeArbLog(u'找不到策略类：%s' %className)
            return
        
        # 防止策略重名
        if name in self.strategyDict:
            self.writeArbLog(u'策略实例重名：%s' %name)
        else:
            # 创建策略实例
    ...
    #----------------------------------------------------------------------
    def updateStrategy(self, name, data):
        """刷新策略组合"""
        if name in self.strategyDict:
            strategy = self.strategyDict[name]
	    if not strategy.arbEngine:
	    	strategy.arbEngine = self
	    self.callStrategyFunc(strategy, strategy.onUpdate, data)
        else:
            self.writeArbLog(u'策略实例不存在：%s' %name)        
	...
	if name in self.strategyDict:
		...
		self.writeArbLog(u'请勿重复初始化策略实例：%s' %name)
	else:
	    self.writeArbLog(u'策略实例不存在：%s' %name)        
    ...
    #---------------------------------------------------------------------
    def sendStrategy(self, name, data):
        """启动策略"""
        if name in self.strategyDict:
            strategy = self.strategyDict[name]
	    self.callStrategyFunc(strategy, strategy.onSend, data)
        else:
            self.writeArbLog(u'策略实例不存在：%s' %name)
    ...
    #----------------------------------------------------------------------
    def clearStrategy(self, name):
        """读取策略配置"""
        if name in self.strategyDict:
            strategy = self.strategyDict[name]
	    self.updateContract(name,strategy)
    ...
    #----------------------------------------------------------------------
    def cancelStrategy(self, name):
        """读取策略配置"""
        if name in self.strategyDict:
            strategy = self.strategyDict[name]
	    self.callStrategyFunc(strategy, strategy.onRedraw)
    	
    	self.putStrategyEvent(name)

    #----------------------------------------------------------------------
    def qryposStrategy(self, name):
        """读取策略配置"""
        if name in self.strategyDict:
            strategy = self.strategyDict[name]
	    self.callStrategyFunc(strategy, strategy.onCalcPos, self.posDict)
    	
    	self.putStrategyEvent(name)

    #----------------------------------------------------------------------
    def reportStrategy(self, name):
        """读取策略配置"""
        if name in self.strategyDict:
            strategy = self.strategyDict[name]
	    strategy.report()
    ...
    #----------------------------------------------------------------------
    def getStrategyPrice(self, name):
        """获取策略当前的变量字典"""
        if name in self.strategyDict:
            strategy = self.strategyDict[name]
            prcDict = OrderedDict()
            
            for key in ['askPrice1','bidPrice1']:
                prcDict[key] = strategy.__getattribute__(key)
            
            return prcDict
        else:
            self.writeArbLog(u'策略实例不存在：' + name)    
            return None
