...
from ctaAlgo.ctaEngine2 import CtaEngine2
...
class MainEngine(object):
    ...
    def __init__(self):
        ...
        self.ctaEngine2 = CtaEngine2(self, self.eventEngine)
        self.registerEvent()
        print 'vtengine init finish'
    ...
    def dbInsert(self, dbName, collectionName, d):
        ...
        if 'datetime' in d.keys():
            if isinstance(d['datetime'],datetime):
                ...
                d['datetime']=datetime.strftime( d['datetime'], '%Y-%m-%d %H:%M:%S.%f')
            #print d['datetime'],type(d['datetime'])
        if self.mysqlConnection:
            sql='insert into stockcn.tb_tick (symbol,ndate,tick) values '
            values = ''

            ndate=d['datetime']
            values = values + '(\'{0}\',\'{1}\',\"{2}\")'.format(
                collectionName,ndate,d)

            #print values
            cur = self.mysqlConnection.cursor(MySQLdb.cursors.DictCursor)

            try:
                cur.execute(sql+values)
                if self.commitcount >100:
                    self.commitcount=0
                    self.mysqlConnection.commit()

                    d2=datetime.strptime(d['datetime'],'%Y-%m-%d %H:%M:%S.%f')
                    print datetime.now(),d2
                    n1=(datetime.now()-d2).seconds
                    self.writeLog(u'%s commit, %s' %(ndate ,n1))
    ...
    def registerEvent(self):
        """注册事件监听"""
        self.eventEngine.register(EVENT_LOG, self.procecssLOG)
        self.eventEngine.register(EVENT_ERROR, self.procecssERROR)
        self.eventEngine.register(EVENT_CTA_LOG, self.procecssCTALOG)

    def procecssLOG(self,event):
        log=event.dict_['data']
        print log.logTime,log.logContent

    def procecssERROR(self,event):
        log=event.dict_['data']
        print log.errorID,log.errorMsg,log.additionalInfo

    def procecssCTALOG(self,event):
        log=event.dict_['data']
        print log.logTime,log.logContent
